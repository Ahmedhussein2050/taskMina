...
class BlogsDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables($query)
            ...
            ->addColumn('edit', function($query){
                $input = url('/article/'.$query->id);
                $domain = preg_replace('#^https?://#', '', rtrim($input,'/'));

                $html = '<a href="#" data-toggle="modal" data-target="#get_link"
                data-link="'.request()->getScheme().'://'.$domain.'" class="btn btn-default get_link"  title="' . _i("Show Link") . '">
                   <i class="ti-link"></i></a>  &nbsp;'.'';
                $html .= '<a href="'. url('admin/blog/'.$query->id.'/edit') .'" id="item_id_' . $query->id . '" class="btn btn-primary"  title="' . _i("Edit") . '">
                   <i class="ti-pencil-alt"></i></a>  &nbsp;'.'
                   <form class=" delete"  action="'.url("admin/blog/".$query->id) .'"  method="POST" id="deleteRow"
    ...
    {
        return Article::leftJoin('articles_data' ,'articles_data.article_id','articles.id')
            ->select('articles.*','articles_data.article_id','articles_data.lang_id','articles_data.source_id',
                'articles_data.title','articles_data.content')->where('articles_data.lang_id' , getLang());
    }
                ...
                'buttons' => [
                    [
                        ...
                        'action' => 'function( e, dt, button, config){
                                                             window.location = "'.url('admin/blog/create').'";
                                                         }',
                    ],
...
//                "language" =>  self::lang(),
    ...
    protected function getColumns()
    {
        return [
            ...
            ['name'=>'edit','data'=>'edit','title'=>_i('Control'),'printable'=>false,'exportable'=>false,'orderable'=>false,'searchable'=>false],
