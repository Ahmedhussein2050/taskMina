...
class ArticleCategoryDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('edit', function($query){
                ...
                $html = '<a href="'. url('master/article_cat/'.$query->id.'/edit') .'" id="item_id_' . $query->id . '" class="btn btn-primary"  title="' . _i("Edit") . '">
                   <i class="ti-pencil-alt"></i></a>  &nbsp;'.'
                   <form class=" delete"  action="'.url("master/article_cat/".$query->id) .'"  method="POST" id="deleteRow"  
    ...
    public function query( )
    {
        ...
        ->select('article_category.*','article_category_data.category_id','article_category_data.lang_id','article_category_data.source_id',
            'article_category_data.title')->where('article_category_data.source_id' , null)->where('store_id',null);
                ...
                'buttons' => [
                    [
                        ...
                        'action' => 'function( e, dt, button, config){
                                                             window.location = "'. url('master/article_cat/create') .'";
                                                         }',
                    ],
