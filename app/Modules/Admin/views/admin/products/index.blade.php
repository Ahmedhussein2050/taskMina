...
@section('content')
    @include('admin.products.includes.product_modal')
    @push('css')
        <style>
            .star-ratings-css {
...
{{--    </div>--}}
    ...
    true,
                ...
                @include('admin.products.translate')
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ _i('Edit Products') }}</h5>
                    ...
                    <form id="edit_form" method="POST" class="j-pro" enctype="multipart/form-data"
                        ...
                        <div class="j-unit">
                            <div class="j-input">
                                ...
                                <textarea placeholder="{{ _i('tags') }}" id="tags1" class="form-control"
                                          name="tags"></textarea>
                            </div>
                            <span class="text-danger">
                        ...
                        <div class="row">
                            <div class="col-md-6 pr-0" data-toggle="tooltip" data-placement="top"
                                ...
                                <div class="form-group">
                                    <input type="text" class="form-control cost mr-2" id="skuEdit" name="sku"
                                           required=""
                                           placeholder="{{ _i('SKU') }}">
                            ...
                            <div class="col-md-6 pr-0" data-toggle="tooltip" data-placement="top"
                                ...
                                <div class="form-group">
                                    <input type="text" class=" form-control price mr-2" id="refidEdit" name="refid"
                                           required="" placeholder="{{ _i('REF ID') }}">
                                    <div class="clearfix"></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="j-unit">
                            <div class="j-input">
                                ...
                                <textarea placeholder="{{ _i('Video') }}" id="video" class="form-control"
                                          name="video"></textarea>
                            </div>
                            <span class="text-danger">
    ...
    <div class="modal fade" id="import-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ _i('Import Products') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="import_form" action="{{ route('products.import') }}" method="POST" class="j-pro" enctype="multipart/form-data"
                        ...
                        <div class="j-unit">
                            <div class="j-input">
                                ...
                                <input type="file" id="import" name="file_products" required>
                            </div>
                            <span class="text-danger">
                                <strong id="price-error"></strong>
                            </span>
                        ...
                        <div class="j-unit">
                            <button type="button" class="btn btn-secondary"
                                    data-dismiss="modal">{{ _i('Close') }}</button>
                            <button type="submit" class="btn btn-primary">{{ _i('Start Uploading') }}</button>
                        </div>
                    </form>
                </div>
...
@push('js')
    ...
    <script type="text/javascript">
        ...
        function info() {
            ...
            var url = "{{ route('products.info.store') }}";
    ...
    <script>
        $('body').on('click', '.lang_ex1', function (e) {
            ...
            $('#langedit #title').val(title);
            $('#langedit #description').val(description);
            $('#langedit #info').val(info);
    ...
    <script>
        ...
        $('body').on('click', '#info_btn', function (e) {
            ...
            $.ajax({
                ...
                {{--    if (res.data && res.data != null && res.data != '' || res.data.length != 0) {--}}
                {{--        $("#info_boxes").empty();--}}

                {{--        var j = 50;--}}
                {{--        var k = 50;--}}
                {{--        $.each(res.data, function(ii, obj) {--}}
                {{--            if (obj.lang_id == 1) {--}}
                {{--                $('#info_boxes-' + obj.lang_id).append(--}}
                {{--                    ' <div class="col-md-12 row clear' + j +--}}
                {{--                    '"><div class="col-md-10"><input type="text" class="form-control" form="add_info_form" value="' +--}}
                {{--                    obj.title + '" name="new_info[' + obj--}}
                {{--                    .lang_id + '][]" ></div>' +--}}
                {{--                    '<div class="col-md-2"><a class="btn btn-danger btn-circle waves-effect waves-light info_remove  "   data-toggle="modal" data-url="{{ url('admin/products_info') }}/' +--}}
                {{--                    obj.id + '" data-id="' + obj.id +--}}
                {{--                    '" data-j="' + j +--}}
                {{--                    '" id="info_remove"><i class="ti-trash"></i></a></div></div>'--}}
                {{--                );--}}
                {{--                j++;--}}
                {{--            } else {--}}
                {{--                $('#info_boxes-' + obj.lang_id).append(--}}
                {{--                    ' <div class="col-md-12 row clear' + k +--}}
                {{--                    '"><div class="col-md-10"><input type="text" class="form-control" form="add_info_form" value="' +--}}
                {{--                    obj.title + '" name="new_info[' + obj--}}
                {{--                    .lang_id + '][]" ></div>' +--}}
                {{--                    '<div class="col-md-2"><a class="btn btn-danger btn-circle waves-effect waves-light info_remove  "     data-toggle="modal" data-url="{{ url('admin/products_info') }}/' +--}}
                ...
                {{--        });--}}
                {{--    } else {--}}
                {{--        var lang1 = res.langs[0].id;--}}
                {{--        var lang2 = res.langs[1].id;--}}
                {{--        $('#info_adding_boxes-' + lang1).append(--}}
                {{--            ' <div class="col-md-12 row remove"> <div class="col-md-10"><input   type="text" class="form-control" placeholder="{{ _i('Set Info Title') }}" name="new_info[' +--}}
                {{--            lang1 +--}}
                {{--            '][]" required=""></div> <div class="col-md-2"><a id="info_remove" data-lang="' +--}}
                {{--            lang1 +--}}
        ...
        $('body').on('click', '#info_add', function (e) {
            ...
            $('#info_adding_boxes-' + lang1).append(
                ...
                '"> <div class="col-md-10"><input   type="text"  class="form-control{{ $errors->has('new_info') ? ' is-invalid' : '' }}" placeholder="{{ _i('Set Info Title') }}" name="new_info[' +
