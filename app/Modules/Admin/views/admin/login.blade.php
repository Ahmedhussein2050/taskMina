...
<head>
    @if ($settings != null)
        <title> {{ $settings->title }} | {{ !empty($title) ? $title : _i('title') }}</title>
    @else
        <title>{{ env('APP_NAME') }} | {{ !empty($title) ? $title : _i('Admin login') }}</title>
    @endif
...
<body class="fix-menu">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <div class="login-card card-block auth-body">
                        ...
                        <form class="md-float-material" method="post" action="{{ route('admin.do.login') }}">
                            @csrf
                            @method('POST')
                            <div class="auth-box">
                                <div class="row m-b-20">
                                    <div class="col-md-12">
                                        ...
                                        <div class=" text-center">
                                            <img src="{{asset('images/MASH.png')}}" alt="logo.png"
                                ...
                                <div class="row m-t-25 text-left">
                                    ...
                                    <div class="col-sm-5 col-xs-12 forgot-phone text-right">
                                        <a href="{{ url('forgetPassword') }}"
                                            class="text-right f-w-600 text-inverse"> {{ _i('forget password') }}
                                            ?</a>
                                    </div>
                                </div>
                                <div class="row m-t-30">
                                    <div class="col-md-12">
                                        <button type="submit"
                                            class="btn btn-primary btn-md btn-block waves-effect text-center m-b-20">{{ _i('login') }}</button>
                                    </div>
                                </div>
                                <hr />
                            </div>
    ...
    <!-- Required Jquery -->
    <script type="text/javascript" src="{{ asset('admin_dashboard/bower_components/jquery/js/jquery.min.js') }}">
    </script>
    ...
    <!---- color js --->
    <script type="text/javascript" src="{{ asset('admin_dashboard/assets/js/common-pages.js') }}"></script>
    @stack('js')
</body>
