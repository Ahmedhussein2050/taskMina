<div class="modal fade bd-example-modal-lg" id="edit-email-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">{{ _i('Edit Item') }}</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form id="edit-email-form" class="j-pro" enctype="multipart/form-data" data-parsley-validate="" style="box-shadow:none; background: none">
					@csrf
					@method('post')
					<div class="j-unit">
						<div class="j-input">
							...
							<input type="text" id="email" name="title" placeholder="{{ _i('Email') }}" required>
						</div>
					...
					<div class='m-b-25'>
						...
						<select name="lists[]" multiple class="selectpicker2 form-control" id="lists">
					...
					<div class="j-unit link_field link">
                        <div class="j-input">
                            <label class="j-icon-left" for="email">
                                <i class="icofont icofont-link"></i>
                            </label>
                            <input type="text" id="link2" name="link" placeholder="{{ _i('link') }}" required>
                        </div>
                        <span class="text-danger">
							<strong id="email-error"></strong>
						</span>
					...
					{{-- <div class='m-b-25 category_field  '>
						<label for="category_id"></label>
                        {!!Form::select('category_id',$categories,null,['class'=>'    form-control ','required'=>'','title'=>_i('SelectCategory')]) !!}
                    </div> --}}
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-dismiss="modal">{{ _i('Close') }}</button>
				<button type="submit" form="edit-email-form" class="btn btn-primary">{{ _i('Save') }}</button>
			</div>
		</div>
	</div>
</div>
