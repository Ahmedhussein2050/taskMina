	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form_add" class="form-horizontal" action="{{url('admin/settings/banners/store')}}" method="POST" data-parsley-validate="" enctype="multipart/form-data">
					@csrf
					<div class="box-body">
						<!-- ================================== lang =================================== -->
						...
						<div class="form-group row" >
							...
							<div class="col-sm-10">
								<select class="form-control" name="place" id="lang_add">
									<option selected disabled><?=_i('CHOOSE')?></option>
										<option value="1"
										<?=old('place') == 1 ? 'selected' : ''?>>{{ _i('Banner #1') }}
										</option>
										<option value="2"
										<?=old('place') == 2 ? 'selected' : ''?>>{{ _i('Banner #2') }}
										</option>
										<option value="3"
										<?=old('place') == 3 ? 'selected' : ''?>>{{ _i('Banner #3') }}
										</option>
                                        <option value="4"
										<?=old('place') == 4 ? 'selected' : ''?>>{{ _i('Banner #4') }}
										</option>
								</select>
							</div>
						</div>
						...
						<div class="form-group row" >
							...
							<div class="col-sm-10">
								<input type="text" id="title_add" class="form-control" name="title" required="" placeholder="{{_i('Place Enter Banner Title')}}">
							</div>
						</div>
						<!-- ================================== title =================================== -->
						<div class="form-group row" >
							...
							<div class="col-sm-10">
								<input type="url" id="link_add" class="form-control" name="link" required="" placeholder="{{_i('Place Enter Banner Link')}}">
							</div>
						</div>
						<!----==========================  published ==========================--->
						<!-- checkbox -->
						...
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<input type="file" name="image" id="image_add" class="btn btn-default" accept="image/*" required="">
	...
	<script>
		$(function() {
			$('#form_add').submit(function(e) {
				e.preventDefault();
				$.ajax({
					url: "{{url('admin/settings/banners/store')}}",
					type: "POST",
					"_token": "{{ csrf_token() }}",
