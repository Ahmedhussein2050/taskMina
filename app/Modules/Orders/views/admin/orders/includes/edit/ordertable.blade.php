...
<div class="order-table col-md-12">
    <div class="card">
        ...
        <div class="card-body">
            @include('.admin.orders.includes.edit.order-table', [
                'products' => $order->orderProducts,"admin"
            ])
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        @if (isset($edit))
                            <div class="col-md-3">
                                <a href="javascript:void(0)" onclick="addProduct()"><i class="ti-plus"></i>
                                    {{ _i('add new product') }}</a>
                            </div>
                        @endif
                    ...
                    @if (isset($edit))
                        <button class="btn btn-tiffany col-md-12" type="button" style="cursor: pointer;"
        ...
        <script language="javascript">
            ...
            function getproduct(index) {
                ...
                $('.product-show').append('<img class="media-object media-right img-responsive" src="' + this.product.photo +
                    '">')
                $('.product-show').append('<div class="media-body media-left"><div>' + this.product.title + '</div>' + '<div>' +
                    this.product.price + '</div></div>')
            ...
            function saveproduct() {
                ...
                $("#productRow_" + product.id + "").append(
                    '<td><button style="float: right;margin-top: 20px;margin-left: 20px;" class="btn btn-danger"onclick="productdel(' +
                    product.id + ')"><span>x</span></button><div class="media" id="media_' + product.id + '">');
                $("#media_" + product.id + "").append('<img class="media-object media-right img-responsive" src="' + product
                    .photo + '">')
                $("#media_" + product.id + "").append('<div class="media-body media-left"><div>' + product.title + '</div>')
                $("#productRow_" + product.id + "").append('</td><td class="addcount" id="addcount' + product.id +
                    '"><slot>1</slot></td>')
                $("#addcount" + product.id + "").append(
                    '<div class="form-group input-fly" style="left: 35%;" v-show="toggled"><input onkeyup="countchange(' +
                    product.id + ')" id="changingcount' + product.id +
                    '" value="1" type="text" class="form-control"  style="width: 40px"></div><a href="javascript:void(0)"  class="count"></a>'
                )
                if (product.options.length == 0) {
                    $("#productRow_" + product.id + "").append(
                        '</td><td><slot><a href="javascript:void(0)" class="changeprice" id="changeprice' + product.id +
                        '">' + product.price + '</a></slot>')
                } else {
                    $("#productRow_" + product.id + "").append(
                        '</td><td><slot><a href="javascript:void(0)" class="changeprice" id="changeprice' + product.id +
                        '">' + product.option.price + product.price + '</a></slot>')
                }
                if (product.options.length == 0) {
                    window.total = this.product.price
                    $("#productRow_" + product.id + "").append('</td><td><slot class="changetotal" id="changetotal' + product
                        .id + '">' + total + '</slot></td>')
                } else {
                ...
                $(".table").append(
                    "<tr class='carttotalrow'><td colspan='3'>{{ _i('cart_total') }}</td><td class='carttotal'></td></tr>"
                )
                $(".table").append(
                    "<tr class='shippingoptionvaluerow'><td colspan='3'>{{ _i('shipping_cost') }}</td><td id='shippingoptionvalue'>{!! $shippingOption ?? 0 !!}</td></tr>"
                )
                $(".table").append(
                    "<tr class='couponsrow'><td colspan='3'>{{ _i('discount_coupons') }}</td><td class='coupons' id='coupons" +
                    product.id + "'></td></tr>")
                $(".table").append(
                    "<tr class='changewholetotalrow'><td colspan='3'>{{ _i('order_total') }}</td><td class='changewholetotal'></td></tr>"
                )
            ...
            function productdel(id) {
                ...
                if (prices.length == 0) {
                    ...
                    prices = prices.map((a) => parseInt(a));
            ...
            function formSubmit() {
                ...
                axios.post('../admin/orders/saveproduct', product)
                    .then((data) => {
                        ...
                        if (product.features != null) {
                            for ([option, index] in product.features) {
                                ...
                                $('.product-show').append('<input type="radio" v-model="selectedFeature" name="' + option
                                    .title + '" value="' + option + '" change="' + selectfeature + '">');
                                $('.product-show').append('<span>' + option.title + '</span></label>');
                                $('.product-show').append(
                                    '<table class="table"><tr><th>Title</th><th>Price</th><th>Count</th></tr>');
                            }
                            for (opt in options.options) {
                                if (opt.count > 0) {
                                    ...
                                    if (selectedFeature == null || selectedFeature.id != opt.feature_id) {
                                        $('.product-show').append(
                                            '<input disabled type="radio" onchange="selectOption" :name="opt.title" :value="opt">'
                                        )
                                    } else {
                                        $('.product-show').append(
                                            '<input type="radio" onchange="selectOption" :name="opt.title" :value="opt">'
                                        )
                                    }
                                    ...
                                    $('.product-show').append('<img class="media-object media-right img-responsive" src="' +
                                        product.photo + '"><div class="media-body media-left">')
                                    $('.product-show').append('<div>' + product.name + '</div><div>' + product.price +
                                        '</div></div></div></div></div>')
                                }
                            }
                            $('.product-show').append(
                                '<div class="col-md-4"><div class="media"><img class="media-object media-right img-responsive" src="' +
                                product.photo + '">')
                            $('.product-show').append('<div class="media-body media-left">')
                            $('.product-show').append('<div>' + productname + '</div><div>' + product.price +
                                '</div></div></div></div>')
                        }
                        let newproduct = data.data;
                        form_data.append('product_id', newproduct.id);
                        axios.post('../admin/imageSubmit', form_data).then((data) => {
            ...
            function productform() {
                ...
                $('.add-product').append(
                    '<div class="row add-product-row"><div class="col-md-6"><div class="form-group titlefield">')
                $('.titlefield').append(
                    '<label for="title"></label><input name="title" required="" class="form-control" id="title" placeholder="{{ _i('product_name') }}"></div>'
                )
                $('.add-product-row').append(
                    '<div class="col-md-6"><div class="form-group"><label for="productType"></label><select id="productType" required="" class="selectpicker form-control">'
                )
                ...
                $('.add-product-row').append(
                    '<div class="col-md-6"><div class="form-group countfield"><label for="count"></label>')
                $('.countfield').append(
                    '<input id="count" required="" class="form-control" placeholder="{{ _i('qty') }}"></div></div>')
                $('.add-product-row').append(
                    '<div class="col-md-6"><div class="form-group pricefield"><label for="price"></label>')
                $('.pricefield').append(
                    '<input id="price" required="" class="form-control" placeholder="{{ _i('price') }}" id="$product->price"></div></div></div>'
                )
                $('.add-product-row').append(
                    '<div class="col-md-6"><label for="image"><input type="file" required="" id="image" name="image"></label></div></div>'
                )
                $('.add-product-row').append(
                    '<div class="col-md-6"><img id="blah" width="250" height="250" style="margin-left:46px" src="/images/placeholder.png" alt="your image" /></div>'
                );
                $('.add-product-row').append(
                    '<br/><div class="col-md-12" style="margin-top: 10px;"><button class="btn btn-primary col-md-6" onclick="formSubmit()">{{ _i('add new    product') }}</button><button class="btn btn-primary col-md-6" onclick="cancel()">{{ _i('cancel') }}</button></div></form>'
                )
            }
            ...
            $(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            for (var i = 0; i < response.data.length; i++) {
                                // if (response.data[i].product_details != null){
                                ...
                                if (response.data[i].product_details.length > 0) {
                                    $('.search-menu').append(
                                        `<li class="prod_id" data-id="${response.data[i].id}">${response.data[i].product_details[0].title}</li>`
                                    )
                                }
                            }
                        }
             ...
             <tr>
                ...
                $('body').on('keyup', '.quantity', function(e) {
                    ...
                    $(this).closest('tr').find('.Subtotal').html($.number(val * price, 2));
                ...
                function calculate() {
                    ...
                    $('.total__befor').html($.number(price, 2));

                    // calculate the shpiing and total
                ...
                $('body').on('click', '.remove-from-cart', function(e) {
                    ...
                    axios.post('{{ url('admin/order/remove') }}', form_data).then((data) => {
                        $(obj).closest('tr').remove();
                        Swal.fire({
                            position: 'top-end',
                            type: 'success',
                            title: "{{ _i('Removed successfully') }}",
                            showConfirmButton: false,
                            timer: 2000
                        });
